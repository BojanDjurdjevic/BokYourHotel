...
@auth
    ...
    <a href="{{ route('bookings.index') }}" class="text-gray-300 hover:text-white">{{ auth()->user()->isSupplier() || auth()->user()->isAdmin() || auth()->user()->isSuperAdmin() ? 'Bookings' : 'My bookings' }}</a>
...
@else
    <a href="{{ route('guest.bookings.find') }}" class="text-gray-300 hover:text-white">Find my booking</a>
