<div class="grid sm:grid-cols-2 lg:grid-cols-6 gap-3">
    <div class="relative sm:col-span-2" x-data="destinationSearch(@js(route('destinations.index')), @js(request('city', '')), @js(request('country', '')))" @click.outside="close()">
        <label for="destination" class="block text-sm mb-1">Destination</label>
        <input id="destination" type="text" x-model="term" @input="changed()" @input.debounce.300ms="search()" @keydown="key($event)"
               role="combobox" aria-autocomplete="list" aria-controls="destination-options" :aria-expanded="open" :aria-activedescendant="active >= 0 && open ? 'destination-' + active : null"
               autocomplete="off" maxlength="64" placeholder="City" class="w-full rounded-xl bg-gray-900 border-gray-700">
        <input type="hidden" name="city" :value="city">
        <input type="hidden" name="country" :value="country">
        <p class="text-sm text-gray-400" aria-live="polite" x-text="loading ? 'Searching destinations…' : error"></p>
        <ul id="destination-options" role="listbox" x-show="open" x-cloak class="absolute z-30 w-full bg-gray-900 border border-gray-600 rounded-xl shadow-lg">
            <template x-for="(item, index) in results" :key="item.city + item.country">
                <li :id="'destination-' + index" role="option" :aria-selected="index === active" @mousedown.prevent="choose(index)" @click="choose(index)"
                    class="p-3 cursor-pointer" :class="index === active ? 'bg-blue-700' : 'hover:bg-gray-800'" x-text="item.city + ', ' + item.country"></li>
            </template>
            <li x-show="!loading && !results.length" class="p-3 text-gray-400" role="presentation">No published destinations found.</li>
        </ul>
    </div>
    @foreach(['check_in' => 'Check-in', 'check_out' => 'Check-out'] as $field => $label)
        <div><label for="{{ $field }}" class="block text-sm mb-1">{{ $label }}</label><input id="{{ $field }}" name="{{ $field }}" type="date" value="{{ request($field) }}" min="{{ now()->toDateString() }}" class="w-full rounded-xl bg-gray-900 border-gray-700"></div>
    @endforeach
    @foreach(['adults' => ['Adults',1,1], 'children' => ['Children',0,0]] as $field => [$label,$default,$min])
        <div><label for="{{ $field }}" class="block text-sm mb-1">{{ $label }}</label><input id="{{ $field }}" name="{{ $field }}" type="number" min="{{ $min }}" max="255" value="{{ request($field, $default) }}" class="w-full rounded-xl bg-gray-900 border-gray-700"></div>
    @endforeach
</div>
@once
<script>
