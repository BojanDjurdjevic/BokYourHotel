<div class="max-w-3xl mx-auto">
    ...
    <div class="bg-gray-900 border border-gray-800 rounded-2xl p-6 space-y-6">
        ...
        @if($staff && $booking->canBeConfirmed() && (! $booking->holdDeadlinePassed() || $booking->payment?->status === \App\Enums\PaymentStatus::Paid))
            <form method="POST" action="{{ route('bookings.confirm', $booking) }}">@csrf<button class="px-6 py-3 rounded-xl bg-green-600 hover:bg-green-500 font-semibold">Confirm booking</button></form>
        @endif
        @if($staff && $booking->canBeCompleted())
            <form method="POST" action="{{ route('bookings.complete', $booking) }}">@csrf<button class="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 font-semibold">Complete booking</button></form>
        @endif
