...
<h2>{{ $booking->hotel->name }}</h2><p>{{ $booking->hotel->address }}<br>{{ $booking->hotel->city }}, {{ $booking->hotel->country }}</p>
...
@if($booking->payment?->status === \App\Enums\PaymentStatus::Refunded)<p>Fake refund recorded. No real bank refund has taken place.</p>@endif
